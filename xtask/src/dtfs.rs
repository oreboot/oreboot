...
pub(crate) fn layout_flash(dir: &Path, path: &Path, areas: Vec<Area>) -> io::Result<()> {
    let mut f = fs::File::create(path)?;
    let mut last_area_end = 0;
    for a in areas {
        info!("Area {:?}: @{:?}, size {:?}", a.name, a.offset, a.size);
        ...
        info!("<{}> @ 0x{last_area_end:x}", a.name);
        ...
        if let Some(path) = &a.file {
            ...
            if data.len() > a.size {
                return Err(io::Error::new(
                    ...
                    format!(
                        "File {image_path:?} is too big to fit into the flash area, file size: {} area size: {}",
                        data.len(),
                        a.size
                    ),
                ));
...
fn read_create() {
    ...
    let image = Path::new("test_image.bin");
    let dtfs_file = "test.dtb";
    ...
    let reference = fs::read(&image).expect("Unable to read testdata file");
