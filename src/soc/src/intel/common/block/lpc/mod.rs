use crate::intel::{
    ...
    common::block::{
        gpmr::{gpmr_write32, GPMR_LPCIOE, GPMR_LPCGMR},
        lpc::{LPC_GENERIC_MEM_RANGE, LPC_IO_ENABLES, LPC_LGMR_ADDR_MASK, LPC_LGMR_EN},
    }
};
use log::error;
use payload::drivers::pci_map_bus_ops::{pci_read_config16, pci_read_config32, pci_write_config16, pci_write_config32};
...
pub fn lpc_open_mmio_window(base: usize, size: usize) {
    ...
    if lgmr & (LPC_LGMR_EN as u32) != 0 {
        error!("LPC: Cannot open window to resource {:x} size {:x}", base, size);
        error!("LPC: MMIO window already in use");
        return;
    }

    if size > LPC_LGMR_WINDOW_SIZE as usize {
        error!("LPC: Resource {:x} size {:x} larger than window({:x})", base, size, LPC_LGMR_WINDOW_SIZE);
    }
