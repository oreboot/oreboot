...
const SMB_UART_CONFIG: *mut u32 = 0xfed8_00fc as *mut _;
...
const FCH_UART_LEGACY_DECODE: *mut u16 = 0xfedc_0020 as *mut _;
...
const IOMUX_BASE: *mut u8 = (ACPI_MMIO_BASE + 0xd00) as *mut _; // Note: 256 u8 block--one u8 per pinctrl
const AOAC_BASE: usize = ACPI_MMIO_BASE + 0x1e00;

// See coreboot:src/soc/amd/common/block/include/amdblocks/aoac.h

const FCH_AOACx40_D3_CONTROL: *mut u8 = (AOAC_BASE + 0x40) as *mut _; // Note: 32 times (control: u8, status: u8) block
...
unsafe fn peek32(a: *mut u32) -> u32 {
    return ptr::read_volatile(a);
}

// Read T from address A, reset bits from RESET_MASK, set bits from SET_MASK, write result back to address A
unsafe fn pokers<T>(a: *mut T, reset_mask: T, set_mask: T) -> ()
where
...
fn d3_control_power_on(n: u8) -> () {
    if n < 32 {
        unsafe {
            pokers(FCH_AOACx40_D3_CONTROL.add(usize::from(n * 2)), 0, AOAC_PWR_ON_DEV);
        }
    }
}
...
fn pinctrl(pin: u8, mode: u8) -> () {
    ...
    unsafe {
        pokers(IOMUX_BASE.add(usize::from(pin)), 3, mode);
    }
}
...
impl Driver for MainBoard {
    fn init(&mut self) -> Result<()> {
        unsafe {
            ...
            poke(FCH_UART_LEGACY_DECODE, FCH_LEGACY_3F8_SH);
            ...
            pokers(0xfed00010 as *mut u32, 0, 8);
