...
fn generate_bindings(oreboot_root: &Path) -> std::io::Result<()> {
    ...
    let include_paths: Vec<PathBuf> = [
        "3rdparty/fsp/ApolloLakeFspBinPkg/Include",
...
fn main() -> std::io::Result<()> {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    println!("cargo:rerun-if-changed={}/src/wrapper.h", manifest_dir);
    ...
    fsp_bin.push("3rdparty/fsp/ApolloLakeFspBinPkg/FspBin/Fsp.fd");
