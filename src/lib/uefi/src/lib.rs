...
pub enum FvTraverseError {
    InvalidFvChecksum { fv_base: usize, checksum: u16 },
    InvalidFfsSize { ffs_base: usize },
    InvalidFfsHeaderChecksum { ffs_base: usize, checksum: u8 },
    InvalidFfsDataChecksum { ffs_base: usize, got_checksum: u8, want_checksum: u8 },
...
impl fmt::Display for FvTraverseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            InvalidFvChecksum { fv_base, checksum } => write!(f, "FV@{:#x} has invalid checksum {:#x}, expected 0x0", fv_base, checksum),
            InvalidFfsSize { ffs_base } => write!(f, "FFS@{:#x} has invalid extended size", ffs_base),
            InvalidFfsHeaderChecksum { ffs_base, checksum } => write!(f, "FFS@{:#x} ffs has invalid checksum {:#x}, expected 0x0", ffs_base, checksum),
            InvalidFfsDataChecksum { ffs_base, got_checksum, want_checksum } => write!(f, "FFS@{:#x} ffs has invalid checksum {:#x}, expected {:#x}", ffs_base, got_checksum, want_checksum),
            OutOfBound { index, limit } => write!(f, "Parsing firmare volume resulted in out-of-bounds access ({:#x} > {:#x})", index, limit),
        }
    }
}
    ...
    F: FnMut(SectionInfo, SectionData<'a>),
{
    ...
    let slice_data = |start, end, limit| {
        if start > data.len() {
            return Err(OutOfBound { index: start, limit: data.len() });
        }
        if end > data.len() {
            return Err(OutOfBound { index: end, limit: data.len() });
        }
        if start > limit {
            return Err(OutOfBound { index: start, limit });
        }
        if end > limit {
    ...
    while index < data.len() {
        let fv_base = index;
        let fv_bytes = slice_data(fv_base, fv_base + size_of::<efi::EFI_FIRMWARE_VOLUME_HEADER>(), data.len())?;
        let fv: efi::EFI_FIRMWARE_VOLUME_HEADER = unsafe { core::ptr::read(fv_bytes.as_ptr() as *const _) };
        ...
        if fv.ExtHeaderOffset == 0 {
            ...
            let fveh_bytes = slice_data(index, index + size_of::<efi::EFI_FIRMWARE_VOLUME_EXT_HEADER>(), fv_end)?;
            let fveh: efi::EFI_FIRMWARE_VOLUME_EXT_HEADER = unsafe { core::ptr::read(fveh_bytes.as_ptr() as *const _) };
        ...
        while {
            ...
            let erase_polarity = if fv.Attributes & efi::EFI_FVB2_ERASE_POLARITY == 0 { 0x00 } else { 0xff };
            let ffs_bytes = slice_data(ffs_base, ffs_base + size_of::<efi::EFI_FFS_FILE_HEADER>(), fv_end)?;
            if ffs_bytes.iter().all(|&x| x == erase_polarity) {
                break; // Reached empty space in FV.
            }

            // Determine the file sizes.
            let ffs: efi::EFI_FFS_FILE_HEADER = unsafe { core::ptr::read(ffs_bytes.as_ptr() as *const _) };
            let (ffs_header_size, ffs_size) = if ffs.Attributes & (efi::FFS_ATTRIB_LARGE_FILE as u8) == 0 {
                ...
                let ffs2_bytes = slice_data(ffs_base, ffs_base + size_of::<efi::EFI_FFS_FILE_HEADER2>(), fv_end)?;
                (ffs2_bytes.len(), unsafe { core::ptr::read(ffs2_bytes.as_ptr() as *const efi::EFI_FFS_FILE_HEADER2) }.ExtendedSize as usize)
            };
            ...
            if ffs.Attributes & (efi::FFS_ATTRIB_CHECKSUM as u8) == 0 {
                if file_checksum != 0xaa {
                    return Err(InvalidFfsDataChecksum { ffs_base, got_checksum: file_checksum, want_checksum: 0xaa });
                }
            } else {
                ...
                if checksum != file_checksum {
                    return Err(InvalidFfsDataChecksum { ffs_base, got_checksum: checksum, want_checksum: file_checksum });
                }
            }
            ...
            if filetypes_with_sections.iter().all(|&x| x != ffs.Type as u32) {
            ...
            while {
                ...
                let sec_base = index;
                let sec_common: efi::EFI_COMMON_SECTION_HEADER = unsafe { core::ptr::read(data[sec_base..].as_ptr() as *const _) };

                // Determine section sizes.
                let (sec_header_size, sec_size) = match little_endian3(sec_common.Size) {
                    0xffffff => (size_of::<efi::EFI_COMMON_SECTION_HEADER2>(), unsafe { core::ptr::read(data[sec_base..].as_ptr() as *const efi::EFI_COMMON_SECTION_HEADER2) }.ExtendedSize as usize),
                    x => (size_of::<efi::EFI_COMMON_SECTION_HEADER>(), x),
                };
                let sec_end = sec_base + sec_size;

                // Apply visitor.
                visitor(SectionInfo { fv_base, ffs_base, ffs_guid: ffs.Name, ffs_type: ffs.Type as u32, sec_base, sec_type: sec_common.Type as u32 }, slice_data(sec_base + sec_header_size, sec_end, sec_end)?);
